...
export class VerificationComponent {
  ...
  verify() {
    ...
    resend() {
      ...
      const apiRequestModel = this.getRequestModel();
      this.onePassService.SendCode(apiRequestModel, guid, language).subscribe(p => {
        if (p) {
          let success, codeSended;
          this.translate.get('Verification.CodeSended').subscribe(p => codeSended = p);
          ...
          this.translate.get('Common.SomethingWrong').subscribe(p => somethingWentWrong = p);
          this.translate.get('Common.Fail').subscribe(p => fail = p);
