...
export class RegisterComponent implements OnInit {
  ...
  bufferValue = 75;
  constructor(private passwordAdvisorService: PasswordAdvisorService, private translate: TranslateService,
  private onePassService: OnePassService, private snackBar: MatSnackBar, private router: Router,
  ...
  accountPassChanged(password: string) {
    ...
    if (password.length < 10) {
      this.translate.get('Register.Short').subscribe(p => this.accountPassStrengthText = p);
    ...
    if (result === PasswordCheckStrength.VeryWeak) {
      this.translate.get('Register.VeryWeak').subscribe(p => this.accountPassStrengthText = p);
      this.accountPassStrength = 10;
    }
    if (result === PasswordCheckStrength.Weak) {
      this.translate.get('Register.Weak').subscribe(p => this.accountPassStrengthText = p);
      this.accountPassStrength = 25;
    }
    if (result === PasswordCheckStrength.Good) {
      this.translate.get('Register.Good').subscribe(p => this.accountPassStrengthText = p);
      this.accountPassStrength = 60;
    }
    if (result === PasswordCheckStrength.Strong) {
      this.translate.get('Register.Strong').subscribe(p => this.accountPassStrengthText = p);
