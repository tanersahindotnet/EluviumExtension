...
export class EditServerComponent implements OnInit, OnChanges {
  ...
  onSubmit() {
    if (this.form.valid) {
      ...
      const encryptedServer = this.encryptionService.encryptServer(server, this.password);

      this.onePassService.AddOrUpdateServer(request, encryptedServer).subscribe(p => {
