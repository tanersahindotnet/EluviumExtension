...
export class OnePassEncryptionService {
  ...
  public decryptPasswords (
    ...
    passwords.forEach(element => {
      passwordList.push({
        ...
        sitePassword: this.encryptionService.decryptAES(
  ...
  public decryptCreditCards (
    passwords: CreditCard[],
    password: string
  ): CreditCard[] {
    ...
    passwords.forEach(element => {
      creditCardList.push({
        ...
        notes: this.encryptionService.decryptAES(element.notes, password),
        title: this.encryptionService.decryptAES(element.title, password),
        cardholderName: this.encryptionService.decryptAES(
  ...
  public encryptCreditCards (
    ...
    ;(creditCardList.id = creditCard.id),
      ...
      (creditCardList.notes = this.encryptionService.encryptAES(
        creditCard.notes,
        password
      )),
      (creditCardList.title = this.encryptionService.encryptAES(
        creditCard.title,
        password
      )),
      (creditCardList.cardholderName = this.encryptionService.encryptAES(
        creditCard.cardholderName,
        password
      )),
      (creditCardList.code = this.encryptionService.encryptAES(
        creditCard.code,
        password
      )),
      (creditCardList.expMonth = this.encryptionService.encryptAES(
        creditCard.expMonth,
        password
      )),
      (creditCardList.expYear = this.encryptionService.encryptAES(
        creditCard.expYear,
        password
      )),
      (creditCardList.number = this.encryptionService.encryptAES(
        creditCard.number,
        password
      ))
    return creditCardList
  ...
  public encryptPassword (passwordItem: Password, password: string): Password {
    ...
    ;(passwordData.id = passwordItem.id),
      ...
      (passwordData.notes = this.encryptionService.encryptAES(
        passwordItem.notes,
        password
      )),
      (passwordData.title = this.encryptionService.encryptAES(
        passwordItem.title,
        password
      )),
      (passwordData.sitePassword = this.encryptionService.encryptAES(
        passwordItem.sitePassword,
        password
      )),
      (passwordData.url = this.encryptionService.encryptAES(
        passwordItem.url,
        password
      )),
      (passwordData.userName = this.encryptionService.encryptAES(
  ...
  public encryptServer (server: Server, password: string): Server {
    ...
    ;(serverData.id = server.id),
      ...
      (serverData.notes = this.encryptionService.encryptAES(
        server.notes,
        password
      )),
      (serverData.title = this.encryptionService.encryptAES(
        server.title,
        password
      )),
      (serverData.userName = this.encryptionService.encryptAES(
        server.userName,
        password
      ))
    serverData.hostName = this.encryptionService.encryptAES(
      server.hostName,
      password
    )
    serverData.password = this.encryptionService.encryptAES(
      server.password,
      password
    )
    return serverData
  ...
  public encryptWifi (
    ...
    ;(wifiData.id = wifiPassword.id),
      ...
      (wifiData.notes = this.encryptionService.encryptAES(
        wifiPassword.notes,
        password
      )),
      (wifiData.title = this.encryptionService.encryptAES(
        wifiPassword.title,
        password
      )),
      (wifiData.password = this.encryptionService.encryptAES(
        wifiPassword.password,
        password
      )),
      (wifiData.ssid = this.encryptionService.encryptAES(
        wifiPassword.ssid,
        password
      ))
    return wifiData
